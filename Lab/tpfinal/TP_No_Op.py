...
class LayoutGraph:
    ...
    def __init__(self, grafo, g, t, iters, refresh, c1, c2, verbose=False):    
        ...
        self.c1 = c1
        self.c2 = c2
        self.k = (math.sqrt((self.ancho*self.largo)/len(self.grafo[0])))
    ...
    def randomize_position(self):
        ...
        for n in vertices:
            ...
            y = random.randint(1,self.largo-1)
            self.pos_x[n] = x
            self.pos_y[n] = y
    ...
    def f_attraction(self):
        ...
        for (v1,v2) in aristas:
            ...
            if(dist < 0.5):
                continue
            fa = (dist**2 / self.k*self.c2)
            fx = fa*(self.pos_x[v2] - self.pos_x[v1]) / dist
            fy = fa*(self.pos_y[v2] - self.pos_y[v1]) / dist
            self.accum_x[v1] += fx
            self.accum_y[v1] += fy
            self.accum_x[v2] -= fx
            self.accum_y[v2] -= fy
        ...
        for n1 in vertices:
            for n2 in vertices:
                ...
                if(dist<1):
                    ...
                    frep = ((self.k*self.c1)**2 / dist)
                    fx = frep*(self.pos_x[n2]-self.pos_x[n1]) / dist
                    fy = frep*(self.pos_y[n2]-self.pos_y[n1]) / dist
                self.accum_x[n2] += fx
                self.accum_y[n2] += fy
        self.info_accum()
    ...
    def f_gravedad(self):
        vertices = self.grafo[0]
        for v in vertices:
            dist = self.calc_dist(self.pos_x[v], self.ancho/2, self.pos_y[v], self.largo/2)
            fx = self.g * (self.ancho/2 - self.pos_x[v]) / dist
            fy = self.g * (self.largo/2 - self.pos_y[v]) / dist
            self.accum_x[v] += fx
            self.accum_y[v] += fy
        self.info_accum()
    ...
    def actual_pos(self):
        ...
        for v in vertices:
            ...
            if(self.pos_x[v]<1):
                self.pos_x[v]=1
            if(self.pos_x[v]>(self.ancho-1)):
                self.pos_x[v]=(self.ancho-1)
                
            self.pos_y[v] = self.pos_y[v] + self.accum_y[v]
            if(self.pos_y[v]<1):
                self.pos_y[v]=1
            if(self.pos_y[v]>(self.largo-1)):
                self.pos_y[v]=(self.largo-1)
        self.info_pos()
...
def lee_grafo_archivo(file_name):
    f = open(file_name, "r")
    ...
    for j in lineas:
        l = j.split()
        aristas.append((str(l[0]), str(l[1])))
